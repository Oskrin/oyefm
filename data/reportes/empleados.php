...
function Header() {   
    ...
    $this->Text(130, 10, utf8_decode('LISTA EMPLEADOS'),1, 'L');
    $this->Text(130, 14, 'OYEFM ECUADOR',1, 'L');

    $this->SetFont('Arial','B',6);
    $this->SetY(30);
    $this->SetX(8);
    $this->multiCell(25, 6, utf8_decode('CÓDIGO'),1,'C');
    ...
    $resp = $class->consulta("SELECT P.codigo_ficha, P.cedula_identificacion, P.nombres_completos, P.telf_fijo, P.telf_celular, P.direccion, P.email FROM corporativo.personal P WHERE estado = '1' order by P.id asc");
    $posiciony = 36;
        ...
        while ($row = $class->fetch_array($resp)) {
            ...
            $this->SetY($posiciony);
            $this->SetX(8);
            $this->multiCell(25,6, utf8_decode($codigo),1);
