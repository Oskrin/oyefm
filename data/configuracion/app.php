<?php 
	...
	if (isset($_POST['session'])) {
		...
		while ($row = $class->fetch_array($resultado)) {
			$data = array('id' => $row[0], 'identificacion' => $row[3], 'nombres' => $row[2], 'telefono' => $row[4], 'celular' => $row[5], 'correo' => $row[6], 'ciudad' => $row[7], 'direccion' => $row[8], 'usuario' => $row[9], 'imagen' => $row[11]);
		}
