<?php 
	...
	if ($_POST['oper'] == "add") {
		$resultado = $class->consulta("SELECT count(*) FROM corporativo.cargos WHERE nombre = '$_POST[nombre]'");
	    ...
	    if ($_POST['oper'] == "edit") {
	    	$resultado = $class->consulta("SELECT count(*) FROM corporativo.cargos WHERE nombre = '$_POST[nombre]' AND id NOT IN ('$_POST[id]')");
			while ($row=$class->fetch_array($resultado)) {
				$data = $row[0];
			}
