angular.module('scotchApp').controller('fotos_personalController', function ($scope, $route) {
	...
	$scope.methodimagen = function(id) { 
		$.ajax({
			url: 'data/fotos_personal/app.php',
			type: 'post',
			data: {llenar_foto:'llenar_foto',id:id},
			dataType: 'json',
			async: true,
			success: function (data) {
				$("#avatar").attr("src","data/fotos_personal/imagenes/"+data.imagen);
			}
		});
			...
			callback: function(result) {
				if(result) {
					$.ajax({
						url: 'data/fotos_personal/app.php',
						type: 'post',
						data: {borrar_personal:'borrar_personal',id:id},
						dataType: 'json',
						async: true,
						success: function (data) {
							var val = data;
		...
		$.ajax({
			...
			success: function(response) { 
				...
				for (var i = 0; i < response.length; i++) {
					...
					var pdf = "<button type='button' class='btn btn-white btn-pink btn-sm'  onclick=\"angular.element(this).scope().methodpdf('"+response[i].id+"')\"><span class='fa fa-file-pdf-o red'> Pdf</button>";
					...
					var acciones =  vizualizar + ' ' + pdf + ' ' + borrar;
