angular.module('scotchApp').controller('conciliacionController', function ($scope, $location,loaddatosSRI,$timeout, $parse, $localStorage) {
	...
	jQuery(function($localStorage) {
		...
		//validacion formulario registro 3
		...
		//validacion formulario registro 4
		$('#form_registro4').validate({
			errorElement: 'div',
			errorClass: 'help-block',
			focusInvalid: false,
			ignore: "",
		...
		// lenar combo bancos 2
		function llenar_select_bancos2() {
			$.ajax({
				url: 'data/pagos/app.php',
				type: 'post',
				data: {llenar_bancos:'llenar_bancos'},
				success: function (data) {
					$('#select_banco2').html(data);
				}
			});
		}
		// 

		// lenar combo bancos 3
		function llenar_select_bancos3() {
			$.ajax({
				url: 'data/pagos/app.php',
				type: 'post',
				data: {llenar_bancos:'llenar_bancos'},
				success: function (data) {
					$('#select_banco3').html(data);
				}
			});
		}
		//  
		...
		// guardar registro 4
		$('#btn_guardar4').click(function() {
			var respuesta = $('#form_registro4').valid();
		...
		$('#btn_0').on('click', function() {
			...
			var formulario2 = $("#form_registro2").serialize();
			var formulario3 = $("#form_registro3").serialize();
			var formulario4 = $("#form_registro4").serialize();

            $.ajax({
		        url: "data/conciliacion/app.php",
		        data: "btn_confirmar=" + submit +"&"+ formulario +"&"+ formulario2 + "&"+ formulario3 +"&"+ formulario4 + "&id=" + id_pago,
		        type: "POST",
		        success: function (data) {
		        	var id = data;
		        	bootbox.alert("Gracias! Conciliacion Confirmada Correctamente!", function() {
	        			var myWindow = window.open('data/reportes/pagos.php?hoja=A5&id='+id,'popup','width=900,height=650');
					  	location.reload();
					});                          
		        }
		    });
		...
		llenar_select_bancos2();
		llenar_select_bancos3();

		// abrir en una nueva ventana reporte
		$scope.methodopdf = function(id) { 
			var myWindow = window.open('data/reportes/pagos.php?id='+id,'popup','width=900,height=650');
		} 
		// fin
	    ...
	    jQuery(grid_selector).jqGrid({	        
	        datatype: "xml",
	        url: 'data/conciliacion/xml_conciliacion.php',      
	        ...
	        colModel:[      
	            ...
	            {name:'estado',index:'estado',frozen : true,align:'left',search:false, hidden: true},
