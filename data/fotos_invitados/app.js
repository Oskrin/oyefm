angular.module('scotchApp').controller('fotos_invitadosController', function ($scope, $route) {
	...
	$scope.methodimagen = function(id) { 
		$.ajax({
			...
			success: function (data) {
				$("#avatar").attr("src","data/fotos_clientes/imagenes/"+data.imagen);
			}
		});
			...
			success: function(response) { 
				...
				for (var i = 0; i < response.length; i++) {
					...
					tabla.row.add([
			            ...
			            response[i]['representante_legal'],
			            response[i]['cedula_representante'],
			            estado,
			            acciones
	                ]).draw(false);                            
		        }
